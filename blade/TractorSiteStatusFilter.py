...
class TractorSiteStatusFilter(TrStatusFilter):
    ...
    def FilterSubprocessOutputLine(self, cmd, textline):
        ...
        elif any([(exception in textline) for exception in self.exceptions]):
            return (self.TR_LOG_FATAL_CODE, -1234, textline)
        else:
            # Extract total progress from stdout
            blender_progress = extract_blender_progress(textline)
            if blender_progress is not None:
                return (self.TR_LOG_ALSO_EMIT + self.TR_LOG_PROGRESS, blender_progress)
