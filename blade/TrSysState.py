...
if sys.platform == "win32":
    from ctypes.wintypes import DWORD, HANDLE, HWND, LPCWSTR, MAX_PATH
...
class TrSysState(object):
        ...
        try:
            if sys.platform == "win32":
                ...
                try:
                    ...
                    num = ctypes.windll.kernel32.GetActiveProcessorCount(
                        ALL_PROCESSOR_GROUP
                    )
                    ...
                    self.logger.debug(
                        "Failed to get proc count from kernel32, fall back to env: "
                        % e.message
                    )
                ...
                try:
                    rc = tr_ctypes_libc.sysctlbyname(
                        "hw.ncpu", ctypes.byref(ncpu), ctypes.byref(size), None, 0
                    )
                except:
                    rc = -1
    ...
    def resolveHostname(self, reflectedAddr=None):
        ...
        try:
            ...
            self.logger.error(
                "cannot resolve network hostname info: " + lnm + " " + str(e)
            )
            hnm = lnm
            aliases = []
            addrs = ["127.0.0.1"]
        ...
        self.logger.info(
            "resolved local hostname: '%s'  %s" % (self.hostname, ",".join(self.addrs))
        )
        if len(self.aliases) > 0:
            self.logger.debug("(aliases: " + ", ".join(self.aliases) + ")")
        ...
        try:
            if sys.platform == "win32":
                ...
                try:
                    ...
                    rc = tr_ctypes_libc.sysctlbyname(
                        "kern.boottime", ctypes.byref(tv), ctypes.byref(tvsz), None, 0
                    )
    ...
    def GetAppTempDir(self):
        ...
        try:
            if sys.platform == "win32":
                ...
                getpath.argtypes = [HWND, ctypes.c_int, HANDLE, DWORD, LPCWSTR]
            ...
            if e[0] not in (errno.EEXIST, errno.ERROR_ALREADY_EXISTS):
                ...
                self.logger.warning("tmpdir %s - %s" % (errclass.__name__, str(excobj)))
    ...
    def resolveCheckpointFilename(self, opts):
        ...
        try:
            ...
            pixarChkpt = "/chkpt." + mh + "_" + str(opts.mport) + "." + bh + ".json"
            out += pixarChkpt
        ...
        try:
            if sys.platform == "win32":
                ...
                ctypes.windll.kernel32.GetDiskFreeSpaceExW(
                    ctypes.c_wchar_p(drivepath), None, None, ctypes.pointer(nbytes)
                )
                ...
                osx_statfs.argtypes = [ctypes.c_char_p, ctypes.POINTER(STRUCT_STATFS)]
                sfs = STRUCT_STATFS()
    ...
    def getPhysicalRAM(self):
        ...
        try:
            if sys.platform == "win32":
                ...
                if 0 == tr_ctypes_libc.sysctlbyname(
                    "hw.memsize", ctypes.byref(ram), ctypes.byref(size), None, 0
                ):
    ...
    def getAvailableRAM_OSX(self):
        ...
        vmstat = subprocess.Popen(
            ["/usr/bin/vm_stat"], stdout=subprocess.PIPE
        ).communicate()[0]
        avail = 0.0
    ...
    def getAvailableRAM_Linux(self):
        ...
        for k in mi.split("\n"):
            ...
            if 3 == len(m) and m[0] in ("MemFree:", "Buffers:", "Cached:"):
                ...
                self.physRAM = math.ceil(float(m[1]) / (1024.0 * 1024.0))
    ...
    def getProcessorUsage_Windows(self):
        ...
        if 0 == ctypes.windll.kernel32.GetSystemTimes(
            ctypes.byref(ftIdle), ctypes.byref(ftKernel), ctypes.byref(ftUser)
        ):
            print (ctypes.WinError())
        else:
    ...
    class TrGPUInfo(object):
            ...
            try:
                if sys.platform == "win32":
                    self.resolveGPU_Windows(
                        gpuEnumByPlatform.get("win32"), gpuExcludePatterns
                    )

                elif sys.platform == "darwin":
                    self.resolveGPU_OSX(
                        gpuEnumByPlatform.get("darwin"), gpuExcludePatterns
                    )

                else:
                    self.resolveGPU_Linux(
                        gpuEnumByPlatform.get("linux"), gpuExcludePatterns
                    )
                ...
                if self.gpuCount > 0:
                    ...
                    if "NVIDIA" in id or "AMD" in id or "ATI" in id or "Intel HD" in id:
                        self.gpuLevel = 2
                        self.gpuTags = "gfx"
        ...
        def resolveGPU_Linux(self, argv, exclusions):
            ...
            r = subprocess.Popen(
                argv, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=useShell
            ).communicate()[0]
            n = 0
            id = None
